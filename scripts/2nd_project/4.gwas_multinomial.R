...
for(i in 1:ncol(matg)) {
  ...
  multinom_mod <- multinom(phenotype ~ ., data = df, model=TRUE)
  
  vv <- car::Anova(multinom_mod, type = "III")
  pvalue <- vv["snp","Pr(>Chisq)"]
  temp <- coef(multinom_mod)
  temp = temp[,"snp"]
  snp_effect_1 = temp[1] ## second level vs first (base)
  snp_effect_2 = temp[2] ## third level vs first (base)
  snp_effect <- as.numeric(multinom_mod$coefficients["snp"])
  
  res = rbind.data.frame(res, data.frame("SNP"=snp_name, "effect_1"=snp_effect_1, "effect_2"=snp_effect_2, "pvalue"=pvalue))
}
...
res <- dplyr::select(res,c(SNP,Chr,Pos,effect_1, effect_2,pvalue))
...
fname = paste(dataset,config$trait,"manhattan_multinomial.pdf",sep="_")
ggsave(filename = paste(config$base_folder,"/results/",fname,sep=""), plot = p, device = "pdf", width = 9, height = 9)

## qq-plot
fname = paste(dataset,config$trait,"qqplot_multinomial.png",sep="_")
