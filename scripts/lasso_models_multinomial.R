...
temp = coef(lasso_fit$finalModel, lasso_fit$finalModel$lambdaOpt)
lasso_coefs = data.frame()
for (ll in temp) {
  
  lasso_coefs = rbind(lasso_coefs,as.data.frame.matrix(ll))
}
lasso_coefs = filter(lasso_coefs, `1` != 0, !(row.names(lasso_coefs) %in% c("(Intercept)")))

writeLines(" - saving model coefficients to 'dictionary'")
fname = paste(config$base_folder, "dict_coefs_multinom.RData", sep="/")
...
if (config$alternative_model == TRUE) {
  ...
  cv_model <- cv.glmnet(x = X_train, y = y_train, alpha = 1, nfolds = config$nfolds, type.measure = "class", family = "multinomial")
  plot(cv_model)
  ...
  best_model <- glmnet(x = X_train, y = y_train, alpha = 1, lambda = best_lambda, family = "multinomial")
  temp = coef(best_model)
  lasso_coefs = data.frame()
  for (ll in temp) {
